...
class PromptFactory:
    ...
    _STAGE1_RULES_DATA: list[dict] = [
        ...
        {"priority": "P1", "text": "游戏内部代码标签保护：方括号内纯英文/数字标识符（如[Bleed]、[SuperCoin]、[OnSucceedAttack]）是引擎代码，必须原样保留，不得翻译或修改其内部内容；即使术语表中存在对应中文名，也应保留方括号内的英文ID，中文显示名应写为'名 '（无括号+尾随半角空格）"},
