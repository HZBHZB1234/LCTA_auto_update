SKILL_DOC = """
# 技能类文本翻译指南
关键格式：所有Buff（状态效果）的翻译默认使用 "中文名称 + 空格"（如`振动 `）；
仅当原文本身就是 `[ID]` 形式且作为纯代码引用时保留 `[ID]`。
同一段译文内两种形式不得混用。名称后的空格不可省略，无论是否处于句尾。
引擎事件标签（[OnSucceedAttack]、[BeforeAttack]、[WhenUse]、[EndSkill]、[OnSucceedAttackHead] 等）必须原样保留英文。
...
*   原ID格式保留：中括号 `[]` 及其内部的内容（ID）原样保留，不进行翻译，即使原文只有这一个id文本。
    ...
    *   译文：[OnSucceedAttack] 使目标增加2级烧伤 强度
*   反例（严禁）：不得把方括号内的英文ID翻译成中文后再塞回括号，即禁止生成 `[中文名]` 形式。
    *   错误：[出血] 、[震颤 ] 、[燃烧]
    *   正确：[Bleed] （保留英文ID）或 出血 （中文显示名，无括号且尾随半角空格）
*   反例（严禁）：不得在正文中保留状态效果的英文ID，应使用术语表中的中文名称。
    *   错误：[Charge]层数 、[Combustion]威力
    *   正确：充能 层数 、烧伤 强度
*   Buff名称后的空格保留：
...
## 七、强度与层数（状态效果的 위력/횟수）
    *   状态效果的 KR原文 `위력`（强度/威力计数）→ 中文统一译为 **"强度"**
    *   状态效果的 KR原文 `횟수`（次数/层数计数）→ 中文统一译为 **"层数"**
    *   注意区分：技能/硬币自身的 `코인 위력`、`스킬 위력` → 译为 **"威力"**（硬币威力、本技能威力），见第三节
*   示例对照：
    *   `[Burst] 위력 4 증가` → `增加4级破裂 强度`
    *   `[Burst] 횟수 4 증가` → `增加4级破裂 层数`
    *   `[Combustion] 위력 2 감소` → `减少2级烧伤 强度`
    *   `[Combustion] 횟수 3 부여` → `施加3层烧伤 。`
